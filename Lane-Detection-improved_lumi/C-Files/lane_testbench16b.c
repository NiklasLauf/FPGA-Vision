...
bmp24_alloc(&image_out,x_size,y_size);
    ...
    for (y=0;y<y_size;y++)
      for (x=0;x<x_size;x++)
        ...
        lum_lt = ((lum_lt >>0)& 0b1111111111111111);
        lum_ct = ((lum_ct >>0)& 0b1111111111111111);
        lum_rt = ((lum_rt >>0)& 0b1111111111111111);
        lum_lc = ((lum_lc >>0)& 0b1111111111111111);
        lum_rc = ((lum_rc >>0)& 0b1111111111111111);
        lum_lb = ((lum_lb >>0)& 0b1111111111111111);
        lum_cb = ((lum_cb >>0)& 0b1111111111111111);
        lum_rb = ((lum_rb >>0)& 0b1111111111111111);
        //if(lum_rb>250){printf("%ld ", lum_rb);}
        ...
        g_sum_2 = (sum_x_2 + sum_y_2)/8192;

        if (g_sum_2 > 8191)
            //g_limit = 8191;
            g_limit = 0b0001111111111111;
        else
            //g_limit = g_square;
            g_limit = ((g_sum_2)& 0b0001111111111111);
            //if(g_limit>8){printf("%d ", g_limit);}
        //g_limit_13bit = 32*(g_limit/32);
        g_int    = floor(sqrt(g_limit*32)/2);

        lum_new = 255 - g_int;
    /*    g_square = g_square_256/8192;
        if (g_square > 8192)
            g_limit = 8192;
        else
            g_limit = g_square;
        g_limit_13bit = 32*(g_limit/32);
        g_int    = sqrt(g_limit)/2;
